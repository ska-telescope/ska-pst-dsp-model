function reshaped = reshape_low_cbf_data(data, n_dim, n_pol, n_chan, verbose_)
  % Reshape LowCBF data, given one-dimensional data from a DADA file
  %
  % Args:
  ...
  for n=1:nheap
      ...
      % packets are in FPT order; reshape returns heap(T,P,F)
      tmp = reshape(heap, [], n_pol, n_chan);

      % check that first dimension has expected number of time samples
      sz = size(tmp);
      assert (sz(1) == nsamp_per_heap);

      % convert x(T,P,F) to x(P,F,T) before copying to result
